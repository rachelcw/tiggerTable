test_gene <- function(allele_name, refrence_allele, data, germline) {
  ...
  # 2
  novel_2 <- findNovelAlleles(sub, germline[refrence_allele] ,v_call="v_call", j_call="j_call",
                            junction="junction", junction_length="junction_length",
                            germline_min=1, seq="sequence_alignment", min_seqs = 1, pos_range = 1:318,  y_intercept = 0.2)
  test_y <- rbind(test_y, novel_2)
  View(test_y)
  }
